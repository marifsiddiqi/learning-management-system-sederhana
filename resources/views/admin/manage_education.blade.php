    ...
    <main>
        <section class="flex flex-col items-center px-28 pb-44 space-y-16">
            <div class="w-[95%] flex justify-between items-center">
                <h1 class="text-5xl font-semibold">Manage Education</h1>
                <a href="/manage-education/create"
                ...
                <table class="table-auto w-[95%] border-separate bg-[#28B8FF] rounded-md">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="py-3 w-2/6">Deskripsi</th>
                    ...
                    <tbody class="bg-[#181727]">
                        ...
                        @foreach ($courses as $course)
                            <tr>
                                ...
                                <td scope="row" class="">
                                    <div class="w-full flex justify-center p-3">
                                        <img src="{{ asset('course/' . $course->gambar_course) }}" alt="Gambar Course"
                                ...
                                <td scope="row">
                                    <p class="flex w-full justify-center p-3 text-justify leading-7 indent-10">
                                        {{ $course->deskripsi_course }}</p>
                                </td>
                                <td scope="row">
                                    <p class="flex w-full justify-center p-3">
                                        {{ \Carbon\Carbon::parse($course->created_at)->format('d-m-Y / H:i') }}</p>
                                </td>
                                <td scope="row">
                                    <div class="flex justify-center items-center p-3 space-x-2">
                                        <a href="/manage-materi/{{ $course->id }}"
                                            class="flex justify-center items-center bg-green-500 px-3 py-1.5 space-x-1.5 rounded-lg">
                                        ...
                                        <a href="/manage-education/edit/{{ $course->id }}"
                                        ...
                                        <form action="/manage-education/delete/{{ $course->id }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
